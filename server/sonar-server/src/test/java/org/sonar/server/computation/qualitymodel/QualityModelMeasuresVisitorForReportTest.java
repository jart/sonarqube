...
public class QualityModelMeasuresVisitorForReportTest {
  ...
  public void compute_effort_to_maintainability_rating_A_measure() throws Exception {
    ...
    int file1Ncloc = 10;
    int file1CodeSmells = 100;
    addRawMeasure(NCLOC_KEY, FILE_1_REF, file1Ncloc);
    addRawMeasure(CODE_SMELLS_KEY, FILE_1_REF, file1CodeSmells);

    int file2Ncloc = 5;
    int file2CodeSmells = 20;
    addRawMeasure(NCLOC_KEY, FILE_2_REF, file2Ncloc);
    addRawMeasure(CODE_SMELLS_KEY, FILE_2_REF, file2CodeSmells);

    int dirCodeSmells = 120;
    addRawMeasure(CODE_SMELLS_KEY, DIRECTORY_REF, dirCodeSmells);

    int moduleCodeSmells = 120;
    addRawMeasure(CODE_SMELLS_KEY, MODULE_REF, moduleCodeSmells);

    int projectCodeSmells = 150;
    addRawMeasure(CODE_SMELLS_KEY, PROJECT_REF, projectCodeSmells);

    underTest.visit(ROOT_PROJECT);

    verifyAddedRawMeasure(FILE_1_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY, (long) (file1CodeSmells - RATING_GRID[0] * file1Ncloc *
      DEV_COST_LANGUAGE_1));
    verifyAddedRawMeasure(FILE_2_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY, (long) (file2CodeSmells - RATING_GRID[0] * file2Ncloc *
      DEV_COST_LANGUAGE_1));
    verifyAddedRawMeasure(DIRECTORY_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY, (long) (dirCodeSmells - RATING_GRID[0] * (file1Ncloc
      + file2Ncloc) * DEV_COST_LANGUAGE_1));
    verifyAddedRawMeasure(MODULE_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY, (long) (moduleCodeSmells - RATING_GRID[0] * (file1Ncloc
      + file2Ncloc) * DEV_COST_LANGUAGE_1));
    verifyAddedRawMeasure(PROJECT_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY, (long) (projectCodeSmells - RATING_GRID[0] *
      (file1Ncloc + file2Ncloc) * DEV_COST_LANGUAGE_1));
  }
  ...
  public void compute_0_effort_to_maintainability_rating_A_when_effort_is_lower_than_dev_cost() throws Exception {
    ...
    addRawMeasure(NCLOC_KEY, FILE_1_REF, 10);
    addRawMeasure(CODE_SMELLS_KEY, FILE_1_REF, 2);
  ...
  public void effort_to_maintainability_rating_A_is_same_as_effort_when_no_dev_cost() throws Exception {
    ...
    addRawMeasure(CODE_SMELLS_KEY, FILE_1_REF, 100);
