...
public class QualityModelMeasuresVisitorForViewsTest {
  ...
  public void compute_effort_to_maintainability_rating_A_measure() throws Exception {
    ...
    int subSubView1CodeSmells = 10000;
    addRawMeasure(CODE_SMELLS_KEY, SUB_SUBVIEW_1_REF, subSubView1CodeSmells);

    int subSubView2CodeSmells = 20000;
    addRawMeasure(CODE_SMELLS_KEY, SUB_SUBVIEW_2_REF, subSubView2CodeSmells);

    int subViewCodeSmells = 30000;
    addRawMeasure(CODE_SMELLS_KEY, SUBVIEW_REF, subViewCodeSmells);

    int viewCodeSmells = 35000;
    addRawMeasure(CODE_SMELLS_KEY, ROOT_REF, viewCodeSmells);

    underTest.visit(treeRootHolder.getRoot());

    verifyRawMeasure(SUB_SUBVIEW_1_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY,
      (long) (subSubView1CodeSmells - RATING_GRID[0] * (projectView1DevCosts + projectView2DevCosts)));
    verifyRawMeasure(SUB_SUBVIEW_2_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY,
      (long) (subSubView2CodeSmells - RATING_GRID[0] * projectView3DevCosts));
    verifyRawMeasure(SUBVIEW_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY,
      (long) (subViewCodeSmells - RATING_GRID[0] * (projectView1DevCosts + projectView2DevCosts + projectView3DevCosts)));
    verifyRawMeasure(ROOT_REF, EFFORT_TO_REACH_MAINTAINABILITY_RATING_A_KEY,
      (long) (viewCodeSmells - RATING_GRID[0] * (projectView1DevCosts + projectView2DevCosts + projectView3DevCosts +
